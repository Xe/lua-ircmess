...
void line_initialize(IRCLine * line, char * source, char * verb, int argc, char ** argv) {
	...
	line->source = calloc(sizeof(char), strlen(source));
	strncpy(line->source, source, strlen(source));

	line->verb = calloc(sizeof(char), strlen(verb));
	strncpy(line->verb, verb, strlen(verb));

	line->argc = argc;

	line->argv = calloc(sizeof(char), argc);

	for(i = 0; i < argc; i++) {
		line->argv[i] = calloc(sizeof(char), strlen(argv[i]));
